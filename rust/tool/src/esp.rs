...
impl EspPaths {
    pub fn new(esp: impl AsRef<Path>, target_arch: Architecture) -> Self {
        let esp = esp.as_ref();
        ...
        let systemd_boot = efi_systemd.join(target_arch.systemd_filename());
        let efi_fallback = efi_efi_fallback_dir.join(target_arch.efi_fallback_filename());
