...
use std::str::FromStr;
use lanzaboote_tool::esp::System;
...
pub fn setup_generation_link(
    ...
    let toplevel = setup_toplevel(tmpdir, &system_double).context("Failed to setup toplevel")?;
    setup_generation_link_from_toplevel(&toplevel, profiles_directory, &system_double, version)
}
...
pub fn setup_toplevel(tmpdir: &Path, system_double: &str) -> Result<PathBuf> {
    let system = System::from_str(&system_double)?;
